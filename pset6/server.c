...
char* indexes(const char* path)
{
    ...
    char* ext2 = "/index.html"; 
    
    char copy[strlen(path)];
    
    char* res;
    
    res = malloc(strlen(path) + strlen(ext1) + 1);
    ...
    res = malloc(strlen(path) + strlen(ext2) + 1);
...
{   
    ...
    if (strcmp("GET", method) != 0 || isalpha(line[3]) != 0 )
    {
        error(405);
        return false;
    }
    ...
    for (int g = 0; g < strlen(path); g++)
    { 
        if (path[g] == 'p' && path[g + 1] == 'l')
        {
            path[g + 1] = '\0';
            break;
    ...
    if (strcmp(controlHTTP, http) != 0 || (line[index] != '\r' && line[index + 1] != '\n') )
    {   
        error(505);
        return false;
    }
